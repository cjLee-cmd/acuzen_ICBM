...
// 나머지 37개 부작용 사례 데이터 직접 삽입
async function insertRemainingCases() {
  try {
    console.log('나머지 부작용 사례 데이터 삽입 시작...');
    ...
    // 나머지 37개 케이스 (Low: 7개, Medium: 10개, High: 15개, Critical: 5개)
    const remainingSeverityDistribution = {
      Low: 7,
      Medium: 10, 
      High: 15,
      Critical: 5
    ...
    const sampleCases = [];
    let caseCounter = 1000; // 고유한 case_number를 위한 카운터

    Object.entries(remainingSeverityDistribution).forEach(([severity, count]) => {
      for (let i = 0; i < count; i++) {
        ...
        const reactionDate = new Date(reportDate.getTime() - Math.random() * (7 * 24 * 60 * 60 * 1000));

        // 고유한 case_number 생성
        const year = new Date().getFullYear();
        const caseNumber = `CSE-${year}-${String(caseCounter).padStart(6, '0')}`;
    ...
    console.log(`생성된 나머지 케이스 분포:`);
    console.log(`- Critical: ${remainingSeverityDistribution.Critical}개`);
    console.log(`- High: ${remainingSeverityDistribution.High}개`);
    console.log(`- Medium: ${remainingSeverityDistribution.Medium}개`);
    console.log(`- Low: ${remainingSeverityDistribution.Low}개`);
    console.log(`총 ${sampleCases.length}개 케이스 생성`);

    // 직접 데이터베이스에 삽입 (배치 삽입)
    try {
      await db.insert(cases).values(sampleCases);
      console.log(`나머지 데이터 삽입 완료: ${sampleCases.length}개의 부작용 사례가 추가되었습니다.`);
    } catch (error) {
      console.error(`데이터 삽입 실패:`, error);
    }

  } catch (error) {
    console.error('나머지 샘플 데이터 삽입 중 오류:', error);
  }
}

insertRemainingCases();